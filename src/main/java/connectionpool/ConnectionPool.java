...
import connectionpool.db.DbParameters;
...
public class ConnectionPool {
    ...
    private ConnectionPool() {
        try {
            ...
            this.driverName = parameters.getDriver();
            this.url = parameters.getUrl();
            this.user = parameters.getUser();
            this.password = parameters.getPassword();
            try {
                this.poolSize = Integer.parseInt(parameters.getPullSize());
            } catch (NumberFormatException e) {
                poolSize = 100;
            }
            initPool();
        } catch (ConnectionPoolException e) {
            log.log(Level.FATAL,"Don't init connection pool. Cause: " + e.getLocalizedMessage());
        }
    }
